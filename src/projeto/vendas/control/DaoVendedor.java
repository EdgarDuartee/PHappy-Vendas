...
public class DaoVendedor {
        ...
        try {
            ...
            while (rs.next() == true) {
                ...
                if (r.next() == true) {
                    ...
                    v.setAtivo(r.getString("ativo"));
                    v.setPermissao(r.getInt("permissao"));
                    v.setDtInicio(r.getString("dtInicio"));
                }
                vendedores.add(v);
            }
        } catch (SQLException ex) {
    ...
    public void desativar(Vendedor vendedor, String inatividade) {
        PreparedStatement ps = null;
        try {
            ps = conn.prepareStatement("update Usuario_Sistema set ativo = ?,dtinatividade = ?"
                    + "Where codigo = ?");

            ps.setString(1, "I");
            ps.setString(2, inatividade);
            ps.setInt(3, vendedor.getCodigo());
    ...
    public ArrayList<Vendedor> listarVendedoresTotal() {
        Vendedor v = null;
        PreparedStatement ps = null;

        try {
            ps = conn.prepareStatement("SELECT CODIGO from Usuario_Sistema");
            ...
            while (rs.next() == true) {
                ...
                if (r.next() == true) {
                    ...
                    v.setPermissao(r.getInt("permissao"));
                    v.setDtInicio(r.getString("dtInicio"));
                    v.setDtInatividade(r.getString("DTINATIVIDADE"));
                }
                vendedores.add(v);
            }
        } catch (SQLException ex) {
    ...
    public int contaClientePFPJ(int codigo) {
        int cont = 0;
        PreparedStatement ps = null;
        try {
